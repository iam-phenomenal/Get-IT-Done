...
const revokeUser = async (req, res)=>{
    ...
    try{
        ...
        if(!userTask){
            return res.status(200).json({message: "User has been deleted with no tasks found"});
        }else{
            return res.status(200).json({message: "User and user's tasks has been deleted"});
        }
    }catch(err){
...
const updatePermission = async (req, res)=>{
    ...
    try{
        ...
        const user = await User.findOneAndUpdate({username: username}, {admin: admin}, {new: true});
        return res.status(200).json({
